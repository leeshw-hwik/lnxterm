...
## 2) 프로젝트 상태 요약
- **현재 버전**: v1.8.4
- **최종 빌드**: 2026-02-14 23:01 (KST)
- **주요 변경**: 환경 변수 기반 자동 로딩/저장 구현
...
7. 환경 변수 기반 데이터 로딩 및 저장 (v1.8.4)
- `.env`에서 문자열 통계 및 자동 명령 사전 설정 로드
- `AUTO_LOAD_MODE`: ALWAYS/IGNORE/CONFIRM 지원
- 메뉴 및 종료 시 설정을 `.env`에 자동 저장 (IGNORE 모드 제외)

## 5) Known Issues / 주의점
- PyInstaller 빌드 경고가 출력될 수 있음:
  - `libxcb-cursor.so.0`, `libtiff.so.5`
- 경고가 있어도 빌드 자체는 성공 가능. 타겟 시스템 라이브러리 설치 상태 점검 필요.
- 자동 명령 고빈도 사용 시 성능 테스트 필요.

## 6) 다음 세션 우선순위
1. `v1.8.4` 릴리스 바이너리 실행 환경(타겟 Linux)에서 Qt 의존성 점검
